...
/**
 * Класс UserInterface представляет собой пользовательский интерфейс для взаимодействия
 * с программой обмена валюты. Он включает методы для получения ввода от пользователя,
 * валидации введенных данных и отображения результатов обмена.
 */
public class UserInterface {
  ...
  /**
   * Метод получения ввода от пользователя для создания объекта ExchangeRequest.
   * Пользователю предлагается ввести сумму, валюту для обмена и валюту для приобретения.
   * В случае некорректного ввода, метод обрабатывает исключение InputMismatchException
   * и предоставляет пользователю возможность ввести корректные данные.
   *
  ...
  /**
   * Приватный метод для валидации введенной пользователем валюты.
   * В случае неверного формата валюты выбрасывается исключение InputMismatchException.
   *
